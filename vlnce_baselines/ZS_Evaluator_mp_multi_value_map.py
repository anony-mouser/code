...
@baseline_registry.register_trainer(name="ZS-Evaluator-mp-multi_value_map")
class ZeroShotVlnEvaluatorMP(BaseTrainer):
    ...
    def _process_llm_reply(self, obs: Observations):
        ...
        self.last_constraint = str(len(self.sub_constraints) - 1)
    ...
    @torch.no_grad()
    def _get_blip_value(self, image: Image, caption: str) -> torch.Tensor:
    ...
    def _prepare_mutli_value_map(self, sub_constraints: dict):
        for id, constraints, in sub_constraints.items():
            if eval(id) < len(sub_constraints) - 1:
                for constraint in constraints:
                    if constraints[0] != "direction constraint":
                        value_map_module = ValueMap(self.config, self.mapping_module.map_shape, self.device)
                        constraint.append(value_map_module)
                    else:
                        constraint.append(None)
            else:
                self.destination_value_map_module = ValueMap(self.config, self.mapping_module.map_shape, self.device)
    
    def _process_multi_value_map(self, sub_constraints: dict, obs: Observations, 
                                 full_map: np.ndarray, full_pose: np.ndarray) -> np.ndarray:
        current_value_map = None
        print("!!!!!!!!", sub_constraints)
        for id, constraints in sub_constraints.items():
            if id == self.last_constraint:
                prompt = self.llm_reply["destination"]
                value_map_module = self.destination_value_map_module
                blip_value = self._get_blip_value(Image.fromarray(obs[0]['rgb']), prompt)
                blip_value = blip_value.detach().cpu().numpy()
                value_map = value_map_module(0, full_map[0], self.floor, self.collision_map, 
                                    blip_value, full_pose[0], self.detected_classes, self.current_episode_id)
                if current_value_map is None:
                    current_value_map = value_map
            else:
                for constraint in constraints:
                    if constraint[0] != "direction constraint":
                        value_map_module = constraint[-1]
                        prompt = constraint[1]
                        blip_value = self._get_blip_value(Image.fromarray(obs[0]['rgb']), prompt)
                        blip_value = blip_value.detach().cpu().numpy()
                        value_map = value_map_module(0, full_map[0], self.floor, self.collision_map, 
    ...
    def _maps_initialization(self):
        ...
        self._prepare_mutli_value_map(self.sub_constraints)
    ...
    def rollout(self):
        ...
        for step in range(12, self.max_step):
            print(f"\nepisode:{self.current_episode_id}, step:{step}, len sub instruction: {len(self.sub_constraints)}")
            ...
            if sum(self.constraints_check) < len(self.sub_instructions):
                ...
                if len(check) == 0:
                    ...
                    self.sub_constraints[str(current_idx)] = current_constraint
                ...
                if start_to_wait and (constraint_steps >= self.min_constraint_steps):
                    if len(self.sub_constraints) > 1:
                        del self.sub_constraints[str(current_idx)]
